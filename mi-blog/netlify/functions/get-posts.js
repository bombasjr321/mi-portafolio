...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    for (const p of candidatePaths) {
      const fetchUrl = `https://api.github.com/repos/${owner}/${repo}/contents/${encodeURIComponent(p)}?ref=${branch}`;
      ...
      if (resp.ok) {
        ...
        return { statusCode: 200, headers: { ...headers, 'Content-Type': 'application/json' }, body: JSON.stringify(Array.isArray(posts) ? posts : []) };
      }
    }
